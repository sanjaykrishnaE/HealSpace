...
function MyAppointment() {
    ...
    const getUserAppointments = async () => {
         try {
            ...
            if (data.success) {
                setAppointments(data.appointments.reverse())   // by using reverse method to get latest appointments on top
            }
         } catch (error) {
            console.log(error);
    ...
    const appointmentRazorpay = async (appointmentId) => {
        try {
            const {data} = await axios.post(backendUrl + '/user/razorpay-payment',{appointmentId},{headers:{token}})
            if (data.success) {
                initPay(data.order)
            }
        } catch (error) {
            ...
            {appointments.map((item,index) => (
                <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
                    ...
                    <div className='flex flex-col gap-2 justify-end'>
                       ...
                       {!item.cancelled && !item.payment && !item.isCompleted && <button onClick={()=> appointmentRazorpay(item._id)} className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-green-500 hover:text-white transition-all duration-300'>Pay Online</button> } 
